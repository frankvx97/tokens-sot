...
export function renderStylus(sections: TokenSection[], options: ExportOptions): string {
  if (!sections.length) {
  ...
  sections.forEach((section, index) => {
    ...
    section.entries.forEach((entry) => {
      const declaration = buildStylusDeclaration(entry, options);
      if (declaration) {
...
function buildStylusDeclaration(entry: TokenSectionEntry, options: ExportOptions): string | null {
  const varName = generateStylusVarName(entry.token, options.casing);
  const aliasName = entry.aliasTarget ? generateStylusVarName(entry.aliasTarget, options.casing) : null;
  const value = aliasName ?? formatTokenValue(entry.mode.value, options);
  if (!value) return null;
  return `${varName} = ${value}`;
}

function generateStylusVarName(token: TokenSectionEntry['token'], casing: ExportOptions['casing']): string {
  const parts: string[] = [];
